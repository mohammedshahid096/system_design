...
const servers = [
  { url: "http://localhost:8001", healthy: true, activeConnections: 0 },
  { url: "http://localhost:8002", healthy: true, activeConnections: 0 },
  { url: "http://localhost:8003", healthy: true, activeConnections: 0 },
];
...
function getNextServer(excluded = new Set()) {
  ...
  console.log(healthyBackends);

  let selectedServer = healthyBackends[0];
  for (const server of healthyBackends) {
    if (server.activeConnections < selectedServer.activeConnections) {
...
function startLoadBalancer() {
  ...
  app.use("/", (req, res, next) => {
    ...
    function attemptProxyRequest() {
      ...
      attempts++;
      backendServer.activeConnections++;

      let released = false; // flag to ensure we only release the connection once
      const release = () => {
        if (!released) {
          released = true;
          backendServer.activeConnections--; // give the slot back, exactly once
        }
      };
      ...
      const proxy = createProxyMiddleware({
        ...
        on: {
          ...
          proxyRes: (proxyRes, req, res) => {
            ...
            console.log(green(`Request to ${backendServer?.url} completed.`));
            release(); // Ensure we release the connection when the request ends
          },
        },
      });
