...
const servers = [
  { url: "http://localhost:8001", healthy: true, weights: 2 },
  { url: "http://localhost:8002", healthy: true, weights: 3 },
  { url: "http://localhost:8003", healthy: true, weights: 1 },
];
...
function getNextServer(excluded = new Set()) {
  ...
  // Expand into a flat weighted list: [A, A, A, A, A, B, C]
  const weightedList = healthyBackends.flatMap((server) =>
    Array(server.weights).fill(server),
  );

  if (currentIndex >= weightedList.length) {
    currentIndex = 0; // Reset index if it exceeds the weighted list size
  }

  let targetServer = weightedList[currentIndex];
